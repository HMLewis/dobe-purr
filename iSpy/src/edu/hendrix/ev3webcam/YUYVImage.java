...
public class YUYVImage {
	...
	public YUYVImage makeSection(YUYVImage img, int x1, int y1, int x2, int y2) {
		x1=x1-(x1%4);
		x2=x2-(x2%4);
		y1=y1-(y1%4);
		y2=y2-(y2%4);
		...
		int l=y2-y1;
		byte[] bytes=img.pix;
		byte[] nb=new byte[2*l*w];
		for (int x=x1; x<=x2; x++) {
			for (int y=y1; y<=y2; y++) {
				int idx= ((y-4)*w*2)+((x-2)*2);
				nb[idx]=bytes[(y*img.width*2)+(x*2)];
				nb[idx+1]=bytes[(y*img.width*2)+(x*2)+1];
			}
		}
		return new YUYVImage(nb, w, l);
	}
}
